...
Given a code change $\mathcal{C}$ including a change in different source code files $[F_1, \dots, F_n]$, where $n$ is a number of files in the code change, we aim to extract salient features for each different file $F_i$. The salient features of each file are then concatenated to each other to represent the features for the given code change. In the rest of this section, we explain how the convolutional networks can extract the salient features for each file in the code change and how these salient features are concatenated. 

Suppose $F_i \in \mathbb{R^{\mathcal{N} \times \mathcal{L}}}$ represents a change in each different file, $\mathcal{N}$ is the number of lines (removals or additions) in a code file, and $\mathcal{L}$ is a sequence of words in each line. For each line $\mathcal{N}_i$, we follow the convolutional networks described in section~\ref{sec:cnn_msg} to extract its features, called $\textbf{z}_{\mathcal{N}_i}$.
